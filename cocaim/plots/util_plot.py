...
from .. import tool_grid, noise_estimator,denoise
from .. import superpixel_analysis as sup

# TODO
# update handles st follow matplotlib conventions
# update/merge colorbar
# update ticks
# merge calls
# merge functions
...
def cn_ranks_plot(ranks,
    ...
    if ax3 is None:
        fig, ax3 = plt.subplots(1, 1, figsize=(d2 * figsize, d1 * figsize))
    ...
    # compression ratio per tile
    if cratio_tile:
        Cplot4 = tool_grid.cn_ranks(dim_block,
                                    np.prod(dim_block,1),
                                    dims[:2],
                                    list_order=list_order)
        Cplot3 = Cplot4/Cplot3
    ...
    if cbar_orientation is not 'horizontal':
    ...
    if text_en:
        ...
        for ii, (row_val, col_val) in enumerate(zip(
            x.flatten(order=list_order),
            y.flatten(order=list_order))):
            c = str(int(Cplot3[int(col_val + 1),
                               int(row_val + 1)] % max_rank))
